...
//     // console.log("Route params:", params); // Debug: see what params contain
//     console.log("Course ID:", courseId);
//   // Ensure your route parameter matches: e.g. /admin/course/:courseId
// //   const id = params.courseId || params.id;

//     const navigate = useNavigate()
//     const dispatch = useDispatch()
//     const {course} = useSelector(store=> store.course)
//     // const selectCourse = course.find(course => course.id === id)
//     const selectCourse = course.find(course => course.id === id);
...
//     //     try {
//     //         const res = await axios.get(`http://localhost:5001/api/v1/course/${id}`, {withCredentials:true})
//     //         if(res.data.success){{
...
//         try {
//             // const res = await axios.get(`http://localhost:5001/api/v1/course/${id}`, { withCredentials: true });
//             const res = await axios.get(`http://localhost:5001/api/v1/course/${courseId}`, { withCredentials: true });
//             if (res.data.success) {
//                 setSelectedCourse(res.data.course);
//             }
//         } catch (error) {
...
//         try {
//             setLoading(true)
//             const res = await axios.put(`http://localhost:5001/api/v1/course/${id}`, formData, {
//                 headers: {
...
//         try {
//             const res = await axios.patch(`http://localhost:5001/api/v1/course/${id}`, {
//                 params: {
//                     action
//                 },
//                 withCredentials:true
...
//                     <Button
//                     onClick={() =>
//                         togglePublishUnpublish(selectedCourse?.isPublished ? "false" : "true")
//                     }
//                     className="bg-gray-800 hover:bg-gray-800"
//                     disabled={!selectedCourse}  // disable if course isn't loaded
//                     >
...
const CourseTab = () => {
    ...
    const dispatch = useDispatch();
    const { course } = useSelector(store => store.course);
    const selectCourse = course.find(course => course.id === courseId);
    ...
    const togglePublishUnpublish = async (action) => {
        try {
            const res = await axios.patch(`http://localhost:5001/api/v1/course/${id}`, { action }, { withCredentials: true });
            if (res.data.success) {
                setPublish(!publish);
                toast.success(res.data.message);
            }
    ...
    return (
        <Card>
            <CardHeader className="flex md:flex-row justify-between">
                ...
                <div className='space-x-2'>
                    <Button onClick={() => togglePublishUnpublish(selectedCourse?.isPublished ? "false" : "true")} className="bg-gray-800 hover:bg-gray-800" disabled={!selectedCourse}>
                        {selectedCourse?.isPublished ? "UnPublish" : "Publish"}
                    </Button>
                    <Button variant="destructive">Remove Course</Button>
                </div>
            ...
            <CardContent>
                <div className='space-y-4 mt-5'>
                    ...
                    <Input value={input.courseTitle} onChange={changeEventHandler} type="text" name="courseTitle" placeholder="Ex. Fullstack developer" />
                    <Label>Subtitle</Label>
                    <Input value={input.subTitle} onChange={changeEventHandler} type="text" name="subTitle" placeholder="Ex. Become a Fullstack developer" />
                    <Label>Description</Label>
                    <RichTextEditor input={input} setInput={setInput} />
                    <Label>Course Thumbnail</Label>
